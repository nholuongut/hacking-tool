...
echo -e "${BLUE}                                    https://github.com/nholuongut/hacking-tool ${NC}"
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading hacking-tool...${NC}"
        if sudo git clone https://github.com/nholuongut/hacking-tool.git.git $install_dir; then
...
#            echo '#!/bin/bash' > hackingtool.sh
            ...
            sudo mv $install_dir/hackingtool.sh $bin_dir/hacking-tool
            echo -e "${GREEN}[✔] Script created successfully [✔]"
        else
            echo -e "${RED}[✘] Failed to download hacking-tool [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (hacking-tool) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
